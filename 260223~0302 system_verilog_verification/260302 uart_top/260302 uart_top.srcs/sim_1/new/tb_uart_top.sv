...
interface uart_interface (
    ...
    property preset_check;
        @(posedge clk) $fell(rst) |=> (uart_rx == 1'b1);
    endproperty
    ...
    assert property (preset_check)
        ...
        $error("%t : [ASSERT] Check Your Reset Initial Value!", $time);
...
class transaction;
    ...
    function void display(string name);
        $display("%t : [%s] tx_data = %2h, rx_data = %2h", $time, name, tx_data, rx_data);
    endfunction 
...
class generator;
    ...
    function new(mailbox#(transaction) gen2drv_mbox, mailbox#(transaction) gen2scb_mbox, event gen_next_ev);
...
class scoreboard;
    ...
    function new(mailbox#(transaction) gen2scb_mbox, mailbox#(transaction) mon2scb_mbox, event gen_next_ev);
        this.gen2scb_mbox = gen2scb_mbox;
        this.mon2scb_mbox = mon2scb_mbox;
        this.gen_next_ev  = gen_next_ev;
    ...
    task run(int run_count); 
        ...
        // 마지막에만 summary 출력
        $display("\n==================================================================");
        $display("%t : [SCB_SUMMARY] TOTAL=%0d PASS=%0d FAIL=%0d",
                 $time, total_count, pass_count, fail_count);
        $display("==================================================================\n");
    endtask // run
