`timescale 1ns / 1ps


module GPIO (
    // BUS Global signal
    input               PCLK,
    input               PRESET,
    // APB Interface signal
    input        [31:0] PADDR,
    input        [31:0] PWDATA,
    input               PWRITE,
    input               PENABLE,
    input               PSEL,
    output logic [31:0] PRDATA,
    output logic        PREADY,
    // external ports
    inout  logic [15:0] GPIO
);
    localparam [11:0] GPIO_CTL_ADDR = 12'h000;
    localparam [11:0] GPIO_ODATA_ADDR = 12'h004;
    localparam [11:0] GPIO_IDATA_ADDR = 12'h008;
    logic [15:0] GPIO_ODATA_REG, GPIO_CTL_REG, GPIO_IDATA_REG;//, GPIO_IDATA_NEXT;
    
    assign PREADY = (PENABLE & PSEL) ? 1'b1 : 1'b0;

    assign PRDATA = (PADDR[11:0] == GPIO_CTL_ADDR) ? {16'h0000, GPIO_CTL_REG} : // 어떤 pin을 쓸건지
        (PADDR[11:0] == GPIO_ODATA_ADDR) ? {16'h0000, GPIO_ODATA_REG}: 
        (PADDR[11:0]) == GPIO_IDATA_ADDR ? {16'h0000, GPIO_IDATA_REG} : 32'hxxxx_xxxx; // LED를 켤지 끌지

    always_ff @(posedge PCLK, posedge PRESET) begin
        if (PRESET) begin
            GPIO_CTL_REG   <= 16'h0000;
            GPIO_ODATA_REG <= 16'h0000;
            //GPIO_IDATA_REG <= 16'h0000;
        end else begin
            if (PREADY) begin
                if (PWRITE) begin
                    case (PADDR[11:0])
                        GPIO_CTL_ADDR:
                        GPIO_CTL_REG <= PWDATA[15:0];  // GPIO CTL REG
                        GPIO_ODATA_ADDR: GPIO_ODATA_REG <= PWDATA[15:0];
                    endcase
                end
//                 else begin
//                    GPIO_IDATA_REG <= GPIO_IDATA_NEXT;
//                end
            end
        end
    end

    gpio U_GPIO (
        .ctl   (GPIO_CTL_REG),
        .o_data(GPIO_ODATA_REG),
        .i_data(GPIO_IDATA_REG),
        .gpio  (GPIO)
    );
endmodule

module gpio (
    input        [15:0] ctl,
    input        [15:0] o_data, // cpu가 준 data
    output logic [15:0] i_data, // cpu에게 줄 data
    inout  logic [15:0] gpio
);
    genvar i;
    generate
        for (i = 0; i < 16; i++) begin
            assign gpio[i]   = ctl[i] ? o_data[i] : 1'bz;
            assign i_data[i] = ~ctl[i] ? gpio[i] : 1'bz;
        end
    endgenerate
endmodule


module APB_GPI (
    // BUS Global signal
    input               PCLK,
    input               PRESET,
    // APB Interface signal
    input        [31:0] PADDR,
    input        [31:0] PWDATA,
    input               PWRITE,
    input               PENABLE,
    input               PSEL,
    input  logic [15:0] GPI,
    output logic [31:0] PRDATA,
    output logic        PREADY
);
    localparam [11:0] GPI_CTL_ADDR = 12'h000;
    localparam [11:0] GPI_IDATA_ADDR = 12'h004;
    logic [15:0] GPI_IDATA_REG, GPI_CTL_REG;
    assign PREADY = (PENABLE & PSEL) ? 1'b1 : 1'b0;
    always_ff @(posedge PCLK, posedge PRESET) begin
        if (PRESET) begin
            GPI_CTL_REG   <= 16'h0000;
        end else begin
            if (PREADY & PWRITE) begin
                case (PADDR[11:0])
                    GPI_CTL_ADDR: GPI_CTL_REG <= PWDATA[15:0];  // GPO CTL REG

                endcase
            end
        end
    end
    assign PRDATA = (PADDR[11:0] == GPI_CTL_ADDR) ? {16'h0000, GPI_CTL_REG} : // 어떤 pin을 쓸건지
        (PADDR[11:0] == GPI_IDATA_ADDR) ? {16'h0000, GPI}: 32'hxxxx_xxxx; // LED를 켤지 끌지
    genvar i;
    generate
        for (i = 0; i < 16; i++) begin
            assign GPI_IDATA_REG[i] = (GPI_CTL_REG[i]) ? GPI[i] : 1'bz;
        end
    endgenerate

endmodule

module APB_GPO (
    // BUS Global signal
    input               PCLK,
    input               PRESET,
    // APB Interface signal
    input        [31:0] PADDR,
    input        [31:0] PWDATA,
    input               PWRITE,
    input               PENABLE,
    input               PSEL,
    output logic [31:0] PRDATA,
    output logic        PREADY,
    output logic [15:0] GPO_OUT
);

    localparam [11:0] GPO_CTL_ADDR = 12'h000;
    localparam [11:0] GPO_ODATA_ADDR = 12'h004;

    logic [15:0] GPO_ODATA_REG, GPO_CTL_REG;

    assign PREADY = (PENABLE & PSEL) ? 1'b1 : 1'b0;

    always_ff @(posedge PCLK, posedge PRESET) begin
        if (PRESET) begin
            GPO_CTL_REG   <= 16'h0000;
            GPO_ODATA_REG <= 16'h0000;
        end else begin
            if (PREADY & PWRITE) begin
                case (PADDR[11:0])
                    GPO_CTL_ADDR:   GPO_CTL_REG <= PWDATA[15:0];  // GPO CTL REG
                    GPO_ODATA_ADDR: GPO_ODATA_REG <= PWDATA[15:0];
                endcase
            end
        end
    end

    assign PRDATA = (PADDR[11:0] == GPO_CTL_ADDR) ? {16'h0000, GPO_CTL_REG} : // 어떤 pin을 쓸건지
        (PADDR[11:0] == GPO_ODATA_ADDR) ? {16'h0000, GPO_ODATA_REG}: 32'hxxxx_xxxx; // LED를 켤지 끌지

    // assign GPO_OUT = (GPO_CTL_REG) ? GPO_ODATA_REG : 16'hzzzz;
    genvar i;
    generate
        for (i = 0; i < 16; i++) begin
            assign GPO_OUT[i] = (GPO_CTL_REG[i]) ? GPO_ODATA_REG[i] : 1'bz; // control register에서 1인것은 DATA가나간다. but 그렇지 않으면 z가나간다.
        end
    endgenerate

endmodule


