...
class i2c_seq_item extends uvm_sequence_item;
	...
	*/
	...
	function string convert2string();
		return $sformatf("[Master] tx_data=0x%0h, rx_data=0x%0h [Slave] tx_data=0x%0h, rx_data=0x%0h, cmd_write=%0b, cmd_read=%0b", tx_data_m,  rx_data_m, tx_data_s, rx_data_s, cmd_write, cmd_read);
	endfunction
endclass
...
class i2c_driver extends uvm_driver #(i2c_seq_item);
	...
	//			r_w = 0;
			...
			`uvm_info(get_type_name(), $sformatf("Master read/write completed : %s", item.convert2string()), UVM_HIGH)

			// stop
			i_if.drv_cb.cmd_stop <= 1'b1;
			@(i_if.drv_cb);
			wait(i_if.drv_cb.done_m);
			`uvm_info(get_type_name(), $sformatf("11111111111 : %s", item.convert2string()), UVM_HIGH)
			i_if.drv_cb.cmd_stop <= 1'b0;
			//while(i_if.done_s!=1'b1) begin @(i_if.drv_cb); end
			//wait(i_if.drv_cb.done_s);
			//@(posedge i_if.done_m);
			//wait (i_if.done_s==1'b0);
			`uvm_info(get_type_name(), $sformatf("22222222 : %s", item.convert2string()), UVM_HIGH)
			@(i_if.drv_cb);
			...
			//i_if.drv_cb.clk_div <= item.clk_div;
			//i_if.tx_data_m <= item.tx_data_m;
			//i_if.tx_data_s <= item.tx_data_s;
			//i_if.start <= 1'b1;

			//wait(i_if.drv_cb.busy_m==1);
			//i_if.start <= 1'b0;
			//`uvm_info(get_type_name(), $sformatf("tx start : %s", item.convert2string()), UVM_HIGH)
			//wait(i_if.drv_cb.busy_s==1);
...
class i2c_monitor extends uvm_monitor;
	...
	virtual task run_phase(uvm_phase phase);
		...
		forever begin
			...
			//wait(i_if.mon_cb.done_m);
			@(posedge i_if.done_s);
			@(i_if.mon_cb);
			@(i_if.mon_cb);
			//wait (i_if.mon_cb.done_s==1);
			...
			// item.cmd_write = i_if.mon_cb.cmd_write;
			// item.cmd_read = i_if.mon_cb.cmd_read;
	...
	function void write(i2c_seq_item item);
		...
		if (item.cmd_write) begin
			if (item.tx_data_m == item.rx_data_s) begin
				`uvm_info(get_type_name(), $sformatf("[PASS] M->S : 0x%0h", item.tx_data_m), UVM_MEDIUM)
				pass_cnt++;
			end else begin
				`uvm_error(get_type_name(), $sformatf("[FAIL] M->S : 0x%0h -> 0x%0h", item.tx_data_m, item.rx_data_s))
				fail_cnt++;
			end
		end else if (item.cmd_read) begin
			if (item.rx_data_m == item.tx_data_s) begin
				`uvm_info(get_type_name(), $sformatf("[PASS] S->M : 0x%0h", item.tx_data_s), UVM_MEDIUM)
				pass_cnt++;
			end else begin
				`uvm_error(get_type_name(), $sformatf("[FAIL] S->M : 0x%0h -> 0x%0h", item.tx_data_s, item.rx_data_m))
				fail_cnt++;
			end
		end else begin
			`uvm_error(get_type_name(), $sformatf("[SCB] !!! ERROR !!! no write/read both"))
			err_cnt++;
		end
...
*/
	...
	virtual function void report_phase(uvm_phase phase);
		...
		`uvm_info(get_type_name(), $sformatf("err_cnt = %0d", err_cnt), UVM_DEBUG)
